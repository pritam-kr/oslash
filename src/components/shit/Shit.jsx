    ...
    return (
        <div className="shits-wrapper flex flex-wrap justify-center mt-2 mb-2 gap-1">
            {each.chairs.map((eachChair) => {
                ...
                const isChair = selectedShits.find((each) => each.selectedShit.id === eachChair.id)
                return (<>{isChair === undefined ? <button
                    key={eachChair.id}
                ...
                </button> : <button onClick={() => chooseAnotherShitHandler(eachChair.id)}
                    key={eachChair.id}
